...
async function loginAndGoto(page) {
  ...
  await bill.payeeNameInput.waitFor({ state: 'visible', timeout: 25000 });
  return bill;
}

// Waits until the from-account dropdown is populated with at least one real option.
async function waitForFromAccount(bill) {
  await bill.fromAccountSelect.waitFor({ state: 'visible', timeout: 20000 });
  await expect(bill.fromAccountSelect.locator('option')).not.toHaveCount(0, { timeout: 20000 });
}
...
test.describe('Block 1 — Page Load', () => {
  ...
  test('TC-BILL-03 | Bill pay page title contains relevant text', async ({ page }) => {
    const bill = await loginAndGoto(page);
    await expect(async () => {
      const text = await bill.getRightPanelText();
      expect(text.toLowerCase()).toMatch(/bill|payment|payee/);
...
test.describe('Block 3 — From Account', () => {
  ...
  test('TC-BILL-10 | From account dropdown loads accounts', async ({ page }) => {
    ...
    const count = await bill.getFromAccountCount();
...
test.describe('Block 4 — Payment Submit', () => {
  test('TC-BILL-11 | Submitting valid payment shows response', async ({ page }) => {
    ...
    await page.waitForLoadState('domcontentloaded');
    await expect(async () => {
      const content = await bill.getRightPanelText();
  ...
  test('TC-BILL-12 | Payment response contains payment related text', async ({ page }) => {
    ...
    await expect(async () => {
      const content = await bill.getRightPanelText();
      expect(content.toLowerCase()).toMatch(/payment|bill|complete|error/);
    }).toPass({ timeout: 20000 });
  });
